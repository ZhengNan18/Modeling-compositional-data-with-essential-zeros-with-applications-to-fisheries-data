...
//  Type sigmaP = exp(log_sigmaP);
  ...
  Type prod_now = 1; 
  for(int i = 0;i < n_age-1;++i){
    prod_now = prod_now*(1+etheta(i));
    p(i) = etheta(i)/prod_now;
  }
  p(n_age-1) = 1/prod_now;

  vector<Type> log_p0 = -lambda0*p;  
  ...
  for(int i = 0;i < n_year;++i){
    ...
    for(int j = 0;j < n_age;++j){
      if(Po(i,j)==0){
        ...
        if(j<last_not0(i)){          
          Po_sum -= Po(i,j);
          del_now(i_not0) = log(Po(i,j))-log(Po_sum)-theta(j);
